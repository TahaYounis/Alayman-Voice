...
data class ResourcesForLiveData<out T> (
    val status: Status,
    ...
    companion object{
        fun <T> success(data: T?) = ResourcesForLiveData(Status.SUCCESS, data, null)
        fun <T> error(message: String, data: T?) = ResourcesForLiveData(Status.ERROR,data,message)
        fun <T> loading(data: T?) = ResourcesForLiveData(Status.LOADING, data,null) // for example if we implement a caching mechanism then could already have the data that comes from th cache while we load the data that comes from the remote data source
    }
}
enum class Status{
    SUCCESS,
