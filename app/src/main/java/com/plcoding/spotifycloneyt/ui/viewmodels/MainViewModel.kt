...
class MainViewModel @Inject constructor(
    ...
    private val _mediaItems = MutableLiveData<ResourcesForLiveData<List<QuranModel>>>()
    val mediaItems: LiveData<ResourcesForLiveData<List<QuranModel>>> = _mediaItems
    ...
    init {
        _mediaItems.postValue(ResourcesForLiveData.loading(null)) // post the loading status because now we start to query this media items

        quranServiceConnection.subscribe(
            MEDIA_ROOT_ID,
            object :
                MediaBrowserCompat.SubscriptionCallback() { // callback to be notified once this subscription is finish
                override fun onChildrenLoaded(
                    ...
                    val items = children.map {
                        QuranModel(
                            // want to go to this list and map this mediaItem to our custom QuranModel objects that our livedata accepts
                            it.mediaId!!,
                    ...
                    _mediaItems.postValue(ResourcesForLiveData.success(items))
                }
            })
    }
    ...
    fun playOrToggleQuran(
        mediaItem: QuranModel,
        toggle: Boolean = false
    ) { // toggle boolean by default we don't toggle the state, if we press on pause button for example toggle will be true, if we tap on the new song toggle will be false because then we want to play that new song
        ...
        if (isPrepared && mediaItem.mediaId ==
            ...
            // get reference to our playback state
    ...
    override fun onCleared() {
        super.onCleared()
        quranServiceConnection.unSubscribe(MEDIA_ROOT_ID,
            object : MediaBrowserCompat.SubscriptionCallback() {}) // empty callback we don't need it
    }
}