...
class ArtistFragment : Fragment(R.layout.fragment_artist) {
    ...
    private var quran: QuranModel? = null
    ...
    private val playListViewModel by viewModels<PlayListViewModel> {
        BaseArtistViewModelFactory(firestore, quran!!.subtitle)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        quranAdapter.setItemClickListener {
            ...
            val sendDataToActivity: SendDataFromFragmentToActivity = activity as SendDataFromFragmentToActivity
            sendDataToActivity.transferredDataFromFragment(quranAdapter.quran, it)

        }
    }

    private fun setupRecyclerView() = rvArtistList.apply {
    ...
    private fun subscribeToObservers() {
            ...
            playListViewModel.artistList.observe(viewLifecycleOwner) { result ->
                when (result.status) {
                    Status.SUCCESS -> {
                        artistProgressBar.isVisible = false
                        result.data?.let {
                    ...
                    Status.LOADING -> artistProgressBar.isVisible = true
                }
        }
    }
